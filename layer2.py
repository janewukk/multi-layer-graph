...
while j <= 12:
	...
	while i < len(data[0]):
		...
		if data[1][i] not in node[data[0][i]]:
			node[data[0][i]][data[1][i]] = [data[4][i]+data[5][i] + data[6][i], data[2][i]+data[3][i]]
		else:
			node[data[0][i]][data[1][i]][0] += data[4][i]+data[5][i] + data[6][i]
	...
	while i < len(data[1]):
		...
		if data[0][i] not in node[data[1][i]]:
			node[data[1][i]][data[0][i]] = [data[9][i]+data[10][i] + data[11][i], data[7][i]+data[8][i]]
		else:
			node[data[1][i]][data[0][i]][0] += data[9][i]+data[10][i] + data[11][i]
			node[data[1][i]][data[0][i]][1] += data[7][i]+data[8][i]
		i += 1

	print("finish create nodes")

	filename = "ByMonthNoWeek/2016/reply_send/2016-"
	if j < 10:
		filename += "0" + str(j)
	else:
		filename += str(j)

	file = open(filename+"-type2", 'w')
