...
while j <= 12:
	...
	while i < len(data[0]):
		...
		if data[1][i] not in node[data[0][i]]:
			node[data[0][i]][data[1][i]] = data[2][i]+data[3][i]
		else:
			node[data[0][i]][data[1][i]] += data[2][i]+data[3][i]
		i += 1
	...
	while i < len(data[1]):
		...
		if data[0][i] not in node[data[1][i]]:
			node[data[1][i]][data[0][i]] = data[7][i]+data[8][i]
		else:
			node[data[1][i]][data[0][i]] += data[7][i]+data[8][i]
		i += 1
	...
	filename = "ByMonthNoWeek/2016/avg_email_week/2016-"
	...
	file = open(filename+"-type1", 'w')
	for id1 in node.keys():
		for id2 in node[id1].keys():
			if node[id1][id2] != 0:
				file.write(str(id1) + "," + str(id2) + "," + str(node[id1][id2] / 4) + "\n")
